...
@api_view(['GET'])
def conversations_start(reuqest, user_id):
    conversations = Conversation.objects.filter(users__in=[user_id]).filter(users__in=[reuqest.user.id])
    ...
    if conversations.count() > 0:
        ...
        conversation.users.add(reuqest.user)
